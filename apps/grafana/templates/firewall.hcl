...
FW_ALLOW_IN="{{ env "attr.unique.network.ip-address" }}:tcp:3000,{{ range nomadService "load-balancer" }}{{ .Address }}:tcp:3000{{ end }},{{ range nomadService "vm-agent" }}{{ if eq .Node (env "node.unique.id") }}{{ .Address }}:tcp:3000{{ end }}{{ end }}"
